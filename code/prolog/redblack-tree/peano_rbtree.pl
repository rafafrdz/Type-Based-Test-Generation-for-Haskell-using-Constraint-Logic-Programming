...
less(X, t(Y,_,_,_), X) :- lt(X,Y).
greater(X, t(Y,_,_,_), X) :- lt(Y,X).
...
countblack(nil, z).
countblack(t(_,b,L,R), Z) :- eq(Z, s(N)), countblack(L, N), countblack(R,N).
countblack(t(_,r,L,R), N) :- countblack(L, N), countblack(R,N).
